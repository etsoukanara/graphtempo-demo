...
if app_mode == "Show instructions":
    ...
    with st.container():
    	...
    	st.write('We provide three real-world datasets for testing the tasks of aggregation and exploration. Alternatively, the user can upload their own dataset providing it follows the appropriate format. The main functions are:')
	...
	with st.sidebar:
		...
		if expl_type == 'Interactions-based':
			...
			if not isinstance(time_variant_attr,list):
				...
				var_domain = [i for i in var_domain if i not in [0, '0']]
			...
			with col1:
				with st.expander('Start Node Value(s)'):
					...
					if var_attrs:
						...
						start_val = st.selectbox(var_attrs[0], sorted([j for j in var_domain]), key=i+'str_2')
			...
			with col2:
				with st.expander('End Node Value(s)'):
					...
					if var_attrs:
						...
						end_val = st.selectbox(var_attrs[0], sorted([i for i in var_domain]), key=i+'stp_2')
			...
			attributes_expl_sky = st.multiselect("Attributes", stc+varying, key='attr_expl')
			# stc_attrs_sky = []
			# var_attrs_sky = []
			...
			stc_attrs_sky = []
			var_attrs_sky = []
			for i in attributes_expl_sky:
				if i in stc:
					stc_attrs_sky.append(i)
				else:
					var_attrs_sky.append(i)

			if stc_attrs_sky and not var_attrs_sky:
				attrtype = 'Static'
			elif not stc_attrs_sky and var_attrs_sky:
				attrtype = 'Variant'
			elif stc_attrs_sky and var_attrs_sky:
				attrtype = 'Mix'
			...
			with col1:
				with st.expander('Start Node Value(s)'):
					start_node = []
					if var_attrs_sky:
						var_lst = sorted([i for i in var_domain])
					...
					if stc_attrs_sky:
						for i in stc_attrs_sky:
							...
							start_val = st.selectbox(i, stc_lst, key = i+'ss')
							start_node.append(start_val)
			with col2:
				with st.expander('End Node Value(s)'):
					end_node = []
					...
					if stc_attrs_sky:
						for i in stc_attrs_sky:
							stc_lst = sorted(list(np.unique(time_invariant_attr[i.lower()].values.flatten())))
							end_val = st.selectbox(i, stc_lst, key = i+'se')
							end_node.append(end_val)
			...
			if event and attr_values_sky and (stc_attrs_sky or var_attrs_sky):
				...
				if event == 'Stability' and stc_attrs_sky and not var_attrs_sky:
					result_sky, dom = Stab_INX_MAX(attr_values_sky,stc_attrs_sky,nodes_df,edges_df,time_invariant_attr)
				elif event == 'Stability' and var_attrs_sky and not stc_attrs_sky:
				...
				elif event == 'Growth' and stc_attrs_sky and not var_attrs_sky:
					result_sky, dom = Growth_UN_MAX(attr_values_sky,stc_attrs_sky,nodes_df,edges_df,time_invariant_attr)
				elif event == 'Growth' and var_attrs_sky and not stc_attrs_sky:
					result_sky, dom = Growth_UN_MAX_var(attr_values_sky,nodes_df,edges_df,time_variant_attr)
				elif event == 'Growth' and stc_attrs_sky and var_attrs_sky:
					result_sky, dom = Growth_UN_MAX_mix(attr_values_sky,stc_attrs_sky,nodes_df,edges_df,time_invariant_attr,time_variant_attr)
				elif event == 'Shrinkage' and stc_attrs_sky and not var_attrs_sky:
					result_sky, dom = Shrink_UN_MIN(attr_values_sky,stc_attrs_sky,nodes_df,edges_df,time_invariant_attr)
				elif event == 'Shrinkage' and var_attrs_sky and not stc_attrs_sky:
					result_sky, dom = Shrink_UN_MIN_var(attr_values_sky,nodes_df,edges_df,time_variant_attr)
				elif event == 'Shrinkage' and stc_attrs_sky and var_attrs_sky:
					result_sky, dom = Shrink_UN_MIN_mix(attr_values_sky,stc_attrs_sky,nodes_df,edges_df,time_invariant_attr,time_variant_attr)
		...
		with st.container():
			...
			if submitted_expl_sky and result_sky:
				...
				with st.expander("3-D visualization"):
					st.write('The x-axis represents past intervals, the y-axis shows reference points, and the z-axis depicts the count of edges at each point, as indicated at the top of each bar.  \n Blue bars represent the top 3 results.')
				...
				with st.expander("2-D visualization"):
					...
					fig1 = px.line(sky_df,
    								...
    								facet_col_wrap=1,
    								#height=600, width=720#height=600, width=1500
    								)
